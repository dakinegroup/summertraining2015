...
ISR(USART_RX_vect)
{
  ...
  if(rxQueue.last_write >= RX_BUFFER_SIZE) {
     tchar = UDR0; //drop this byte;
     UDR0 = tchar;
  } else {
  	rxQueue.buffer[rxQueue.last_write] = UDR0;
     UDR0 = rxQueue.buffer[rxQueue.last_write];
  	rxQueue.last_write++;
  }
  sei();
}
...
int USART_Receive_String2(char *buff, int len) {
	int i = 0;
	cli();
  if(rxQueue.last_read < rxQueue.last_write) {
  	//there is data to be read
  	for(i = 0; 
  		(i < len-1) && ((i+rxQueue.last_read) < RX_BUFFER_SIZE-1); 
  		i++) {
  		buff[i] = rxQueue.buffer[rxQueue.last_read+i];
  	}
  	buff[i] = 0;
  	rxQueue.last_read = i + rxQueue.last_read;
  } else {
  	// either it is same, in which case both the pointers should be reduced to zero
	rxQueue.last_read = rxQueue.last_write = 0;  	
  }
  sei();
}
